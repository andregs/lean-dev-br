...
class DestinationControllerWebMvcTest {
  ...
  @Test
  void get_upstreamUnavailable_returnsBadGatewayProblemJson() {
    when(catalog.find("any-id"))
        .thenThrow(new UpstreamUnavailableException("catalog unreachable", new RuntimeException()));

    mvc.get().uri("/destinations/{id}", "any-id")
        .assertThat()
        .hasStatus(502)
        .hasContentType(MediaType.APPLICATION_PROBLEM_JSON)
        .bodyJson()
