...
describe('CartPage', () => {
  ...
  it('reflects items added before the cart page ever mounted', () => {
    // The cart route is lazy-loaded (as a separate remote here), so items are
    // routinely added to the bus before CartPage exists at all — the bus's
    // replay-on-subscribe is what reconstructs the full cart once it finally
    // mounts, whether that's a lazy import within one bundle (modulith) or a
    // remote loaded later over the network (this twin).
